...
**Strict mode, asymmetrically applied.** `backup.sh` and `verify.sh` use full bash strict mode (`set -euo pipefail`): `-e` exits on any error, `-u` treats unset variables as errors, and `-o pipefail` catches failures in piped commands. For these two scripts, an unexpected non-zero exit *should* halt execution — a half-completed backup or a verifier that silently skipped checks is worse than a loud abort.

**`restore.sh` deliberately drops `-e` and runs with `set -uo pipefail` only.** A restore is fundamentally different from a backup: it performs dozens of *optional* operations across ~17 phases — copying maybe-missing dotfiles, brew-installing already-installed packages, pip-installing wheels that need Xcode CLT for C extensions, chmod'ing files the upstream backup didn't capture. Treating any single non-zero exit as fatal means one app's broken backup sinks an entire multi-hour run. We hit this five different ways during real-world use (`.zprofile` shellenv duplication, `*.pub` nullglob trap, empty Zed app-settings dir, pip3 partial wheel-build failure, SSH agent xattr copy on a non-APFS source). The fix is structural: let optional ops fail with a `warn` and keep going; the few critical ops (`brew install`, `brew bundle`) get explicit `if`-checks where it actually matters. `set -u` is kept (catches typo'd variable references) and `pipefail` is kept (don't silently lose pipeline failures).

**Strict-mode pitfalls.** For the scripts that *do* run with `-e`, strict mode has three traps that produce silent script aborts mid-phase. New code touching `backup.sh` or `verify.sh` must avoid them:
